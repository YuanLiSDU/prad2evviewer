...
int main(int argc, char *argv[])
{
                            ...
                            if (center_mod) {
                                ...
                                for (const auto &pk : valid_peaks) {
                                    ...
                                    if (std::abs(dx) <= half_win && std::abs(dy) <= half_win){
                                        if(std::abs(pk.time - valid_peaks[0].time) <= 20.f)
                                            E5x5 += pk.energy;
                                    }
                                }

                                int midx = res->hycal.id_to_index(valid_peaks[0].module_id);
                                if (midx >= 0 && midx < (int)res->module_hists.size())
                                    res->module_hists[midx]->Fill(E5x5);
                                res->hit_pos->Fill(center_mod->x, center_mod->y);
                                res->h_E_1cl->Fill(E5x5);
                                float theta = std::atan(std::sqrt(center_mod->x * center_mod->x +
                                                                  center_mod->y * center_mod->y) / hycal_z)
                                              * 180.f / 3.14159265f;
    ...
    for (int m = 0; m < nmod; ++m) {
        ...
        if (peak <= 0 || sigma <= 0 || sigma > 5 * 0.026*peak || chi2 >= 10.f) {
